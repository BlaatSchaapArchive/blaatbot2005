...
// action, still strip the chr(1);
        ...
        if not (command='') then
            ...
            // Why was I ignoring CTCP VERSION from *Serv. I should be replying to StatServ
            if (inchannel = false) and (line = ( CHR(1) +'VERSION' + CHR(1))) then
                ...
                if not (contains(user,'serv')) then
                    begin
...
if ( command = '!music' ) and (ReadParams(data,0,false) ='list') then
  ...
  ListStations();
                    ...
                    // Batch file???? Oh dear, what was I doing back then?!?!?!
                ...
                // End code meow
                    ...
                    if (command = '!pet') and ( data = 'cat' ) then
                        begin say(irc_nick  + char(39) + 's cat: prrrr') end;
                    ...
                    if ( command = '!info' ){and  (AnsiLowerCase(data) = AnsiLowerCase(form1.Nick.text)) }then
                        begin
                        announce('I am BlaatBot2005. My Source Code is available at github');
                        announce('https://github.com/BlaatSchaapArchive/BlaatBot2005');
                    ...
                    // Downloading an update from Nuky's server won't work
                    // Nuky and his server are long gone.
                    {
