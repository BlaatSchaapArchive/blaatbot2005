    ...
    // like the one i used in the quiz project
        ...
        while not eof(badwordsfile) do
            ...
            repeat
                ...
                if not (temp = ',') then
                    begin badword := badword + temp end
            until temp = ',';
            say (badword);
            end;
        ...
        //6
                    ...
                    repeat
                        ...
                        if tempchar = '_' then
                            begin tempchar := ' ' end;
                        if tempchar = '<' then
                            begin stop :=true; tempchar:= char(0); end;
//        if tempchar = '' then stop := true ;//test extra
                    ...
                    begin say ( 'error connecting to ' + station) end;
//end infoshecker
        ...
        while not eof(adminfile) do
            ...
            repeat
                ...
                if not (temp = ',') then
                    begin admin := admin + temp end
            until temp = ',';
            if AnsiLowerCase(name) = AnsiLowerCase(admin) then
                begin result := true end;
            if name = 'áðµíñ' then
                begin result := true end; // GUI INPUT
            end;
        closefile(adminfile);
...
//if (tempstring = what) then temp_contains := true else temp_contains := false;
                ...
                end end; // end check for valid data
...
procedure randomquote();
    ...
    begin
        ...
        repeat
            ...
            while (not eof(myfile)) and (qcounter < randomI) do
                begin
    ...
    begin
        ...
        while not eof(badwordsfile) do
            ...
            repeat
                read (badwordsfile, temp);
                if not (temp = ',') then
                    begin badwordinfile := badwordinfile + temp end
            until temp = ',';
...
procedure CheckForUpdate();
    ...
    begin
        ...
        if http_connect() then
            ...
            if temp2 = '200 OK' then
                begin doorgaan := true end ;
            repeat
                temp:=http_Receiveln();
            until temp =''; // header
            if doorgaan then
     ...
     // new upgrade checker code //
                ...
                if (new_d > cur_d) and (new_m = cur_m) and (new_y = cur_y) then
                    begin upgrade := true end;
                if (new_m > cur_m) and (new_y = cur_y) then
                    begin upgrade := true end;
                if (new_y > cur_y) then
                    begin upgrade:=true end;
                ...
                if upgrade then
                    ...
                    begin say ('Current version = '+ currentversion+' New version = '+newversion  );
